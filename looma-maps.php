...
<div id="main-container-horizontal" class='scroll'>
    ...
    <div class="center">
        ...
        foreach ($maps as $map) {
            // Ensure city street maps (Kathmandu, Pokhara, ...) are accessible even if flagged hidden.
            // These maps use `tileLayer: streetMap` (served from OpenStreetMap in looma-map.js).
            if (isset($map['tileLayer']) && $map['tileLayer'] === 'streetMap') {
                $id = $map['_id'];
                if (!isset($shown_ids[(string)$id])) {
                    echo "<td>";
                    if (isset($map['title'])) $dn = $map['title']; else $dn = "Map";
                    if (isset($map['thumb'])) $thumb = "../content/maps/mapThumbs/" . $map['thumb']; else $thumb = 'images/maps.png';
                    makeMapButton($id, $thumb, $dn);
