...
//////////////////////////////////////
   ...
   $('#timeline').on('mouseout', '.activityDiv',  function () { //handlerOut
    ...
    // Click directly inside the text container to start editing — change text,
    // delete text, etc. — without having to find the "Edit text file" button.
    // Already-editing previews are left alone so the click can place the caret
    // normally. Delegated on #previewpanel so it works for every textpreview
    // the preview() function renders (inline, text, text-template).
    $('#previewpanel').on('click', '.textpreview', function () {
