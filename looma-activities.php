...
looma_trace_page('activities', [
    'ch'        => $_GET['ch']           ?? null,
    'chdn'      => $_GET['chdn']         ?? null,
    'language'  => $_GET['chapter_lang'] ?? null,
        ...
        if ( $ft !== 'text' && $ft !== 'quiz') {
            ...
            switch ($ft) {
                ...
                case "looma";  //open a Looma page (e.g. calculator or paint)
                    ...
                    break;
                case "exercise":
                    // AI-generated exercises (chapter quiz) — published from
                    // the AI page. mongoID == quiz generated_content id; the
                    // player resolves it back via /quiz_data?chapter_id=...
                    $exerciseDn = $dn ?: 'AI Exercises';
                    $exerciseThumb = 'images/quiz.png';
                    if (!file_exists($exerciseThumb)) $exerciseThumb = 'images/lesson.png';
                    makeActivityButton('exercise', null, null, $exerciseDn, '',
                        $exerciseThumb, $ch_id, $mongoID, '', '',
                        '', '', '', '', null, null, null, $lang);
                    break;
                case "vocab":
                case "voc":
                    // Legacy: any leftover ft='vocab' Mongo activity stubs.
                    // The chapter vocab game is now rendered unconditionally
                    // for every chapter (see the synthetic button below the
                    // foreach loop), so we just skip the activity stub here
                    // to avoid showing two buttons for the same thing.
                    break;
                case "ep":
                case "epaath":
...
*/
 ...
 //       echo "<span class='tip yes-show big-show' >Chapter " . $ch_id . "</span>";
        ...
        // Exams used to live here per-chapter, but they're always grade-wide.
        // The "Exams" entry point is now on looma-chapters.php (top-right
        // button) → looma-exams-list.php. Removed from each chapter's
        // Resources page to avoid the duplicate, per-chapter button.
