...
function logPageHit($page) {
    ...
    // Mirror to OpenTelemetry so the Grafana "Page & Chapter Engagement"
    // dashboard receives a `looma_event_total{looma_event="page", looma_page=...}`
    // counter for every PHP page hit (not just the JS-instrumented PDF viewer).
    looma_emit_page_metric($page);
} // end logPageHit()

function looma_emit_page_metric($page) {
    ...
    // Use the request start time as the start of the cumulative counter so the
    // Prometheus exporter never emits a 0-second-window data point that gets dropped.
    $startNanos = isset($_SERVER['REQUEST_TIME_FLOAT'])
        ? (string)((int)round(((float)$_SERVER['REQUEST_TIME_FLOAT'] - 60) * 1e9))
        : (string)((int)$tNanos - 60_000_000_000);

    $attrs = [
        ['key' => 'looma.event', 'value' => ['stringValue' => 'page']],
        ['key' => 'looma.page',  'value' => ['stringValue' => (string)$page]],
    ];
    $payload = [
        'resourceMetrics' => [[
            'resource' => ['attributes' => [
    ...
    if (function_exists('curl_init')) {
        $ch = curl_init($endpoint . '/v1/metrics');
...
function logUserActivity () {
    ...
    // Log on every environment except the offline 'looma' boxes (which run
    // without an internet uplink — the ipinfo geo lookup in logUser would
    // hang). Local dev still logs so the /activity page has data to plot.
    if ($LOOMA_SERVER !== 'looma') {
