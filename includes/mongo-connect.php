...
// Ensure all endpoints touching MongoDB are traceable, including AJAX utilities
// that do not include the full page header bootstrap.
...
function mongoCount($collection, $query) {
    global $mongo_level;
    $span = _looma_mongo_span('count', $collection, $query);
    ...
    if ($span && function_exists('looma_otel_end_span')) looma_otel_end_span($span);
    return $count;
}
...
function _looma_mongo_collection_name($collection) {
    if (is_object($collection) && method_exists($collection, 'getCollectionName')) {
        try { return (string) $collection->getCollectionName(); } catch (Throwable $e) {}
    }
    return 'unknown';
}

function _looma_mongo_span($op, $collection, $filter = null) {
    if (!function_exists('looma_otel_start_span')) return null;
    return looma_otel_start_span(
        'mongo.' . $op,
        [
            'db.system'      => 'mongodb',
            'db.operation'   => $op,
            'db.mongodb.collection' => _looma_mongo_collection_name($collection),
            'db.statement'   => $filter ? json_encode($filter) : null,
        ],
        3 // CLIENT
    );
}
...
function mongoFind($collection, $filter, $sort, $skip, $limit) {
    ...
    $span = _looma_mongo_span('find', $collection, $filter);
    ...
    if ($span && function_exists('looma_otel_end_span')) {
        looma_otel_end_span($span);
    }
    return $cursor;
}
...
function mongoFindOne($collection, $filter) {
    ...
    $span = _looma_mongo_span('findOne', $collection, $filter);
...
function mongoFindRandom($collection, $filter, $count) {
    ...
    global $mongo_level;
    $span = _looma_mongo_span('findRandom', $collection, $filter);
    if ($mongo_level >= 3) {
...
function mongoDistinct($collection, $key) {
    global $mongo_level;
    $span = _looma_mongo_span('distinct', $collection, ['key' => $key]);
...
function mongoFindAndModify($collection, $filter, $set) {
    ...
    $span = _looma_mongo_span('findAndModify', $collection, $filter);
    if ($mongo_level >= 3 ) {
...
function mongoInsert($collection, $doc) {
    global $mongo_level, $logcollections;
    if (!loggedIn() && !in_array($collection, $logcollections)  ) return null;
    $span = _looma_mongo_span('insert', $collection);
...
function mongoUpdate($collection, $filter, $set) {
    global $mongo_level, $logcollections;
    if (!loggedIn() && !in_array($collection, $logcollections)  ) return null;
    $span = _looma_mongo_span('update', $collection, $filter);
